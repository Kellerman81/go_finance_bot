...
func (y *YahooHistory) Candles(ctx context.Context, symbol string, res Resolution, from, to time.Time) ([]Candle, error) {
...
func (y *YahooHistory) fetch(ctx context.Context, symbol, interval, rng string) (_ []Candle, err error) {
	defer func() {
		if err != nil {
...
func (y *YahooHistory) fetchOnce(ctx context.Context, host, symbol, interval, rng string) (_ []Candle, retryable bool, err error) {
	...
	for i, ts := range r.Timestamp {
		...
		if i >= len(q.Close) || q.Close[i] == nil || q.Open[i] == nil || q.High[i] == nil || q.Low[i] == nil {
			continue
		}
		var vol float64
