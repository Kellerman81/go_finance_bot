...
func NewTrading212(baseURL, apiKey, secret string, isins map[string]string, preferCcy string) *Trading212 {
	if preferCcy == "" {
		preferCcy = "EUR"
	}
	if isins == nil {
...
func (t *Trading212) Name() string { return "trading212" }
...
func (t *Trading212) pace(ctx context.Context, group string) error {
	t.rlMu.Lock()
	defer t.rlMu.Unlock()
	min, ok := t212Limits[group]
	if !ok {
		min = t212Limits["default"]
	}
	if wait := min - time.Since(t.lastCall[group]); wait > 0 {
...
func (t *Trading212) do(ctx context.Context, method, path, group string, body, out any) (err error) {
	defer func() {
	...
	case resp.StatusCode == http.StatusUnauthorized:
		return fmt.Errorf("trading212 %s %s: 401 unauthorized (check api_key/secret and that base_url matches the key's environment)", method, path)
	case resp.StatusCode == http.StatusTooManyRequests:
		return fmt.Errorf("trading212 %s %s: 429 rate limited", method, path)
	case resp.StatusCode < 200 || resp.StatusCode >= 300:
		return fmt.Errorf("trading212 %s %s: status %d: %s", method, path, resp.StatusCode, string(data))
	}
...
func (t *Trading212) loadInstruments(ctx context.Context) error {
	...
	if err := t.do(ctx, http.MethodGet, "/equity/metadata/instruments", "instruments", nil, &raw); err != nil {
		return fmt.Errorf("load instruments: %w", err)
	}
	byShort := make(map[string][]t212Inst, len(raw))
	byISIN := make(map[string][]t212Inst, len(raw))
	for _, in := range raw {
		inst := t212Inst{Ticker: in.Ticker, ShortName: in.ShortName, ISIN: in.ISIN, Currency: in.CurrencyCode}
...
func pickListing(candidates []t212Inst, country, ccy string) t212Inst {
	...
	case country == "US":
		...
		for _, c := range candidates {
			if strings.Contains(c.Ticker, "_US_") || strings.HasPrefix(strings.ToUpper(c.ISIN), "US") {
				us = append(us, c)
			}
		}
		if len(us) > 0 {
...
func (t *Trading212) SubmitOrder(ctx context.Context, o Order) (OrderResult, error) {
	...
	for prec >= 0 {
		...
		if qty <= 0 {
			return OrderResult{}, fmt.Errorf("trading212: quantity %v too small at precision %d", o.Qty, prec)
		}
...
// sells.
func (t *Trading212) postMarketOrder(ctx context.Context, ticker, symbol string, side Side, qty float64) (OrderResult, error) {
	signed := qty
	if side == Sell {
		signed = -qty
	}
	...
	var resp t212Order
	if err := t.do(ctx, http.MethodPost, "/equity/orders/market", "orders", body, &resp); err != nil {
		return OrderResult{}, err
	}
...
func (t *Trading212) CancelOrders(ctx context.Context, ids ...string) error {
	if len(ids) == 0 {
		return fmt.Errorf("trading212: no order ids to cancel")
	}
	for _, id := range ids {
		if err := t.do(ctx, http.MethodDelete, "/equity/orders/"+id, "orders", nil, nil); err != nil {
			return err
		}
	}
	return nil
