...
func VWAP(high, low, close, volume []float64, period int) []float64 {
	n := len(close)
...
func VWAPLast(high, low, close, volume []float64, period int) (float64, bool) {
	n := len(close)
	if period <= 0 || n < period {
		return 0, false
	}
...
func BuildVolumeProfile(close, volume []float64, window, buckets int, valueAreaPct float64) (VolumeProfile, bool) {
	n := len(close)
	...
	for covered < target && (low > 0 || high < buckets-1) {
		...
		if above >= below && high < buckets-1 {
			high++
			covered += vol[high]
		} else if low > 0 {
			low--
			covered += vol[low]
		} else if high < buckets-1 {
			high++
			covered += vol[high]
		} else {
			break
