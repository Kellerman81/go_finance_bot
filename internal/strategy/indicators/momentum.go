...
// the given periods. %K = 100·(close − lowestLow) / (highestHigh − lowestLow)
...
func StochasticLast(high, low, close []float64, kPeriod, dPeriod int) (k, d float64, ok bool) {
	n := len(close)
	...
	kAt := func(i int) (float64, bool) {
		...
		return 100 * (close[i] - lo) / rng, true
	}
	last, ok := kAt(n - 1)
	if !ok {
		return 0, 0, false
