...
//
//nolint:cyclop // header mapping + per-row parsing is one linear pass
func LoadCSV(path string) (map[string][]market.Candle, error) {
	...
	for sym := range out {
		series := out[sym]
		slices.SortFunc(series, func(a, b market.Candle) int { return a.Time.Compare(b.Time) })
...
func SaveCSV(path string, data map[string][]market.Candle) error {
	...
	for _, sym := range symbols {
		...
		slices.SortFunc(candles, func(a, b market.Candle) int { return a.Time.Compare(b.Time) })
...
// ftoa formats a float with the fewest digits that round-trip, for CSV output.
...
// parseTime accepts RFC3339, "2006-01-02 15:04:05", a bare date, or unix seconds.
...
// atof parses a float, returning 0 on malformed input.
