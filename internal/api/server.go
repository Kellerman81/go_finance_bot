...
func New(eng *engine.Engine, rm *risk.Manager, pf *portfolio.Portfolio, prices *engine.PriceCache, authToken string) *Server {
...
func (s *Server) getBehavior(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"keep_interval": keep.String(), "confirm_buy": cb, "confirm_sell": cs})
}
...
func (s *Server) putBehavior(c *gin.Context) {
	...
	if keep < 0 || req.ConfirmBuy < 1 || req.ConfirmSell < 1 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "keep_interval >= 0 and confirm_buy/confirm_sell >= 1 required"})
		return
	}
...
func (s *Server) postBracket(c *gin.Context) {
	...
	side := broker.Side(strings.ToLower(strings.TrimSpace(req.Side)))
	if (side != broker.Buy && side != broker.Sell) || req.Qty <= 0 || strings.TrimSpace(req.Symbol) == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "require symbol, side (buy|sell), qty>0"})
		return
	}
