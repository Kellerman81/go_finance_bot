...
func New(cfg config.EngineConfig, exits config.ExitsConfig, safety config.SafetyConfig, stratCfg config.StrategyConfig,
	p market.DataProvider, b broker.Broker, rm *risk.Manager, pf *portfolio.Portfolio, prices *PriceCache, store Store, log *slog.Logger) *Engine {
	if log == nil {
		log = slog.Default()
	}
	if store == nil {
...
type HistorySource interface {
	Candles(ctx context.Context, symbol string, res market.Resolution, from, to time.Time) ([]market.Candle, error)
}
...
func (e *Engine) warmup(ctx context.Context, symbol string) {
	...
	if (err != nil || len(candles) == 0) && e.history != nil {
		...
		if c, err2 := e.provider.Candles(cctx, symbol, market.Res1Min, from, to); err2 == nil && len(c) > 0 {
			candles, err = c, nil
		}
	}
...
func (e *Engine) evaluate(ctx context.Context) {
	...
	results := e.evaluateSignals(strat, symbols)
...
func (e *Engine) evaluateSignals(strat strategy.Strategy, symbols []string) []evalResult {
...
// reconcileTTL throttles broker open-order polling.
const reconcileTTL = 15 * time.Second
...
func (e *Engine) reconcile(ctx context.Context) {
	...
	e.mu.RLock()
	fresh := time.Since(e.lastReconcile) < reconcileTTL
	e.mu.RUnlock()
	if fresh {
		return
	}
...
func (e *Engine) act(ctx context.Context, sig strategy.Signal, series []market.Candle, tradingEnabled bool) {
	...
	if e.cfg.MarketHoursCheck {
		if ok, reason := market.OrderingAllowed(sig.Symbol, now(), e.cfg.PreOpenWindow); !ok {
			e.log.Debug("order skipped: market closed", "symbol", sig.Symbol, "side", sig.Action, "reason", reason)
			return
		}
	}
	...
	if err != nil {
		...
		e.record(TradeRecord{
			Time: time.Now(), Symbol: sig.Symbol, Side: authOrder.Side, Qty: authOrder.Qty,
			Price: price, Status: "error", Reason: decision.Reason, Err: err.Error(), Indicators: indSnap,
		})
		return
	}
	...
	e.record(TradeRecord{
		Time: time.Now(), Symbol: sig.Symbol, Side: authOrder.Side, Qty: res.FilledQty,
		Price: fillPx, Value: value, Status: orStr(res.Status, "filled"),
		Reason: sig.Reason + " | " + decision.Reason, OrderID: res.ID,
...
func (e *Engine) confirmVenuePrice(symbol string, signalPrice float64) (price float64, reason string, ok bool) {
	if !e.cfg.VenuePriceCheck {
...
func (e *Engine) ManualOrder(ctx context.Context, symbol string, side broker.Side, qty float64) (TradeRecord, error) {
	...
	authOrder, decision := e.rm.Authorize(order, price, snap)
	if !decision.Approved {
		rec := TradeRecord{Time: time.Now(), Symbol: symbol, Side: side, Price: price,
			Status: "blocked", Reason: decision.Reason, Indicators: indSnap}
		e.record(rec)
		return rec, nil
	}
	...
	res, err := e.brk.SubmitOrder(ctx, authOrder)
	if err != nil {
		rec := TradeRecord{Time: time.Now(), Symbol: symbol, Side: side, Qty: authOrder.Qty,
			Price: price, Status: "error", Reason: decision.Reason, Err: err.Error(), Indicators: indSnap}
		e.record(rec)
		return rec, err
	}
	...
	rec := TradeRecord{Time: time.Now(), Symbol: symbol, Side: side, Qty: res.FilledQty,
		Price: fillPx, Value: value, Status: orStr(res.Status, "filled"),
		Reason: "manual | " + decision.Reason, OrderID: res.ID,
		Indicators: indSnap, PnL: pnlVal, PnLPct: pnlPctVal, Win: win, EntryIndicators: entryIndicators}
	e.record(rec)
	if side == broker.Buy {
...
func (e *Engine) recordOpen(symbol string, qty, price float64, entryIndicators string) {
	...
	if !ok {
		p = &OpenPosition{Symbol: symbol, EntryPrice: price, EntryTime: now(), Peak: price, EntryIndicators: entryIndicators}
		e.openPos[symbol] = p
	} else {
		newQty := p.Qty + qty
		if newQty > 0 {
...
func (e *Engine) SetStrategyConfig(cfg config.StrategyConfig) {
	...
	e.strat = s
	e.mu.Unlock()
	e.log.Info("strategy updated", "detectors", cfg.Detectors, "combine", cfg.Combine, "min_strength", cfg.MinStrength)
}
...
func (e *Engine) SetExitsConfig(x config.ExitsConfig) {
	...
	e.exits = x
	e.mu.Unlock()
	e.log.Info("exits updated", "stop_loss", x.StopLossPct, "trailing", x.TrailingStopPct, "take_profit", x.TakeProfitPct)
}
