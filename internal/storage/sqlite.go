...
func (s *SQLite) LoadDailySpend() (string, float64, error) {
	...
	if _, err := fmt.Sscanf(v, "%10s:%f", &day, &amount); err != nil {
		return "", 0, nil // tolerate malformed value
	}
...
func (s *SQLite) SaveOpenPosition(p engine.OpenPosition) error {
	_, err := s.db.Exec(
		...
		p.Symbol, p.Qty, p.EntryPrice, p.EntryTime.Format(time.RFC3339Nano), p.Peak, p.EntryIndicators)
	return err
}

func (s *SQLite) DeleteOpenPosition(symbol string) error {
	_, err := s.db.Exec(`DELETE FROM open_positions WHERE symbol=?`, symbol)
	return err
}

func (s *SQLite) LoadOpenPositions() ([]engine.OpenPosition, error) {
	rows, err := s.db.Query(`SELECT symbol, qty, entry_price, entry_time, peak, entry_indicators FROM open_positions`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var p engine.OpenPosition
		var ts string
		var entryIndicators sql.NullString
		if err := rows.Scan(&p.Symbol, &p.Qty, &p.EntryPrice, &ts, &p.Peak, &entryIndicators); err != nil {
			return nil, err
		}
...
func (s *SQLite) TradeStats(since time.Time) (engine.TradeStats, error) {
	...
	var stats engine.TradeStats
	var winPnL, lossPnL sql.NullFloat64
	err := s.db.QueryRow(
		`SELECT COUNT(*),
	...
	for rows.Next() {
		...
		if err := rows.Scan(&sym.Symbol, &sym.Trades, &sym.Wins, &sym.Losses, &sym.TotalPnL); err != nil {
			return engine.TradeStats{}, err
		}
		if sym.Trades > 0 {
