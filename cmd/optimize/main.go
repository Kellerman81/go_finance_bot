...
func main() {
	cfgPath := flag.String("config", "data/config.yaml", "base config (fixed indicator periods, limits, costs)")
	...
	respectLimits := flag.Bool("respect-limits", false, "keep config money limits (else relax so deployment isn't capped)")
	random := flag.Int("random", 0, "random-search N samples over the full space INCL. indicator periods (0 = grid search)")
	seed := flag.Int64("seed", 1, "random seed (for reproducible -random runs)")
	sensitivity := flag.Bool("sensitivity", false, "one-factor-at-a-time: change each parameter alone and show the result")
	...
	if *sensitivity {
		fmt.Printf("Sensitivity analysis (one parameter changed at a time, baseline deploy=%.0f%%, limits=%s)\n\n",
			*deploy*100, limMode)
	...
	if *random > 0 {
		fmt.Printf("Random search: %d samples over the full space incl. indicator periods (rank=%s, min-trades=%d, limits=%s, seed=%d)...\n\n",
			*random, *rank, *minTrades, limMode, *seed)
		trials = backtest.OptimizeRandom(cfg, data, *cash, *random, *minTrades, backtest.ScorerByName(*rank), *respectLimits, *seed)
	} else {
		...
		trials = backtest.Optimize(cfg, data, grid, *cash, *minTrades, backtest.ScorerByName(*rank), *respectLimits)
	}
	if len(trials) == 0 {
		fmt.Println("No configurations met the minimum-trades threshold. Try -min-trades 1 or different data.")
		return
	}
...
func printLeaderboard(all []backtest.Trial, top int) {
	...
	fmt.Printf("Ranked %d valid configs (%d distinct outcomes). Top %d:\n\n", len(all), len(trials), top)
...
func printSensitivity(base backtest.Result, axes []backtest.AxisSweep) {
	...
	fmt.Printf("(Δret = change in total return vs baseline when ONLY this parameter changes)\n")
	for _, ax := range axes {
		fmt.Printf("\n%s:\n", ax.Axis)
		for _, r := range ax.Rows {
			...
			fmt.Printf("  %s%-8s  return %7.2f%%  Δret %+6.2f  win %5.1f%%  trades %3d  sharpe %5.2f\n",
				mark, r.Label, r.Result.TotalReturnPct, r.Result.TotalReturnPct-base.TotalReturnPct,
				r.Result.WinRatePct, r.Result.NumTrades, r.Result.Sharpe)
		}
	}
...
func printBest(t backtest.Trial, cash float64) {
	...
	fmt.Printf("  macd_fast: %d\n  macd_slow: %d\n  macd_signal: %d\n", p.MACDFast, p.MACDSlow, p.MACDSignal)
	...
	fmt.Printf("  vp_window: %d\n  vp_buckets: %d\n  vp_value_area_pct: %.2f\n", p.VPWindow, p.VPBuckets, p.VPValueAreaPct)
	fmt.Printf("  trend_period: %d\n  trend_threshold: %.2f\n  trend_gate: %v\n", p.TrendPeriod, p.TrendThreshold, p.TrendGate)
	fmt.Printf("engine:\n")
	fmt.Printf("  order_size_usd: %.0f   # = %.0f%% of %.0f cash\n", t.OrderSize, t.CapitalFrac*100, cash)
	fmt.Printf("exits:\n")
	...
	fmt.Printf("# To realise this, money limits must allow the deployment, e.g.:\n")
	fmt.Printf("# limits: { max_order_value: %.0f, max_per_position: %.0f, max_total_invested: <your cap> }\n",
		t.OrderSize*1.05, t.OrderSize*1.05)
	fmt.Printf("\nResult: return %.2f%%  (buy&hold %.2f%%), win %.1f%%, %d trades, maxDD %.2f%%, sharpe %.2f\n",
		t.Result.TotalReturnPct, t.Result.BuyHoldReturnPct, t.Result.WinRatePct, t.Result.NumTrades,
		t.Result.MaxDrawdownPct, t.Result.Sharpe)
}
