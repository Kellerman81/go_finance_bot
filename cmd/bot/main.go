...
func main() {
	...
	if cfg.Finnhub.APIKey != "" {
		provider = market.NewFinnhub(cfg.Finnhub.APIKey, cfg.Finnhub.RESTBase, cfg.Finnhub.WSBase, log)
	...
	var history engine.HistorySource
	...
	case "saxo":
		...
		sx, err := broker.NewSaxo(cfg.Saxo.BaseURL, tokens, cfg.Saxo.AccountKey, cfg.Saxo.AssetTypes)
		if err != nil {
	...
	case "trading212":
		brk = broker.NewTrading212(cfg.Trading212.BaseURL, cfg.Trading212.APIKey, cfg.Trading212.Secret,
			cfg.Trading212.ISINs, cfg.Trading212.PreferredCurrency)
	...
	eng := engine.New(cfg.Engine, cfg.Exits, cfg.Safety, cfg.Strategy, provider, brk, rm, pf, prices, store, log)
	defer eng.Close()
	if history != nil {
